...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    } else if ($submissionid = onetoone_user_signup($session, $onetoone,
            $course, $fromform->discountcode, $fromform->notificationtype, $statuscode)) {
...
if ($session->datetimeknown && onetoone_has_session_started($session, $timenow)) {
    $inprogressstr = get_string('cannotsignupsessioninprogress', 'onetoone');
    $overstr = get_string('cannotsignupsessionover', 'onetoone');

    $errorstring = onetoone_is_session_in_progress($session, $timenow) ? $inprogressstr : $overstr;
...
if ($signedup) {
    if (!($session->datetimeknown && onetoone_has_session_started($session, $timenow))) {
        // Cancellation link.
        echo html_writer::link(new moodle_url('cancelsignup.php',
                array('s' => $session->id, 'backtoallsessions' => $backtoallsessions)), get_string('cancelbooking', 'onetoone'),
                array('title' => get_string('cancelbooking', 'onetoone')));
        echo ' &ndash; ';
    }
    // See attendees link.
    if ($viewattendees) {
        echo html_writer::link(new moodle_url('attendees.php',
                array('s' => $session->id, 'backtoallsessions' => $backtoallsessions)), get_string('seeattendees', 'onetoone'),
                array('title' => get_string('seeattendees', 'onetoone')));
    }

    echo html_writer::empty_tag('br') . html_writer::link($returnurl, get_string('goback', 'onetoone'),
            array('title' => get_string('goback', 'onetoone')));
} else if (onetoone_manager_needed($onetoone) && !onetoone_get_manageremail($USER->id)) {
