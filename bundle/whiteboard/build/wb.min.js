var io={cfg:{},sock:null,wsuri:null,response:{},auth:!1,error:null,uniquesids:null,init:function(g,a){this.cfg=g;this.wsconnect()},wsconnect:function(){io.wsuri="wss://"+this.cfg.rid;"WebSocket"in window?this.sock=new WebSocket(io.wsuri):"MozWebSocket"in window?this.sock=new MozWebSocket(io.wsuri):(console.log("Browser does not support WebSocket!"),this.error="Browser does not support WebSocket!");var g=this;this.sock.onopen=function(a){$.event.trigger({type:"connectionopen"});g.userauthenticat();
g.addclient()};this.sock.onmessage=function(a){try{var b=JSON.parse(a.data);if("joinroom"==b.type){var c=null;null!=g.uniquesids&&$.each(b.clientids,function(a,b){void 0==g.uniquesids[a]&&(c=a)});g.uniquesids=b.clientids;$.event.trigger({type:"member_added",message:b.users,newuser:c})}if("broadcast"==b.type){var d="";void 0!=b.userto&&(d=b.userto);$.event.trigger({type:"newmessage",message:b.m,fromUser:b.user,toUser:d})}"userleft"==b.type&&(console.log("user logout"),d="",void 0!=b.userto&&(d=b.userto),
delete g.uniquesids[b.user.userid],$.event.trigger({type:"user_logout",fromUser:b.user,message:"offline",toUser:d}));"leftroom"==b.type&&(console.log("member_removed"),$.event.trigger({type:"member_removed",message:b.users}));"Unauthenticated"==b.type&&console.log("Unauthenticated user")}catch(f){console.log("Error   : "+f)}};this.sock.onerror=function(a){g.error=a};this.sock.onclose=function(a){$.event.trigger({type:"connectionclose",message:a.reason});console.log("Connection closed (wasClean = "+
a.wasClean+", code = "+a.code+", reason = '"+a.reason+"')");setTimeout(function(){g.wsconnect()},5E3)}},userauthenticat:function(){console.log("Start Auth");var g=JSON.stringify({cfun:"authenticate",arg:{authuser:this.cfg.authuser,authpass:this.cfg.authpass}});this.sock.send(g)},addclient:function(){var g=JSON.stringify({cfun:"joinroom",arg:{client:this.cfg.userid,roomname:this.cfg.fastchatroom_name,user:this.cfg.userobj}});this.sock.send(g)},send:function(g){var a={cfun:"broadcast",arg:{msg:g}};
1<arguments.length&&(a.arg.touser=this.uniquesids[arguments[1]]);a=JSON.stringify(a);this.sock.send(a)},disconnect:function(){this.sock.onclose=function(){};this.sock.close()}};/*
 2013-2014
*/
...
a.fillStyle=d.overlayFill:d.fill&&(a.fillStyle=d.fill);d.stroke&&(a.strokeStyle=d.stroke);a.beginPath();var k;k=h=0;for(var p=-(d.width/2),g=-(d.height/2),m=0,l=d.path.length;m<l;++m)switch(k=d.path[m],k[0]){case "L":h=k[1];k=k[2];a.lineTo(h+p,k+g);break;case "M":h=k[1],k=k[2],a.moveTo(h+p,k+g)}d.fill&&a.fill();d.stroke&&(a.strokeStyle=d.stroke,a.lineWidth=d.strokeWidth,a.lineCap=a.lineJoin="round",a.stroke());!f&&d.active&&(d.drawBorders(a),d.hideCorners||d.drawCorners(a));a.restore()},makeStackObj:function(a,
b,f,h){return{mt:a,ac:b,x:f,y:h}}};g.vcan=b})(window,document);(function(g,a){var b={tool:"",obj:{},prvObj:"",replayTime:0,sentPackets:0,sentPackDiv:"sentPacket",sentPackDivPS:"sentPacketPS",receivedPackets:0,receivedPackDiv:"receivedNumber",receivedPackDivPS:"receivedNumberPS",uid:0,lt:"",commandToolsWrapperId:"commandToolsWrapper",error:[],view:{},lang:{},system:{},gObj:{},bridge:{},user:{},init:function(){b.vcan=g.vcan;var c=b.vcan;b.canvas=c.create("#canvas");c=c.main.canvas;c.setAttribute("tabindex","0");c.focus();c.onkeydown=b.utility.keyOperation;b.system.setCanvasDimension();
...
b.dataInfo=parseInt(wbUser.dataInfo);f&&1==b.dataInfo&&!b.utility.alreadyExistPacketContainer()&&(b.createPacketContainer(),b.createPacketInfoContainer(),b.utility.initStoredPacketsNumbers())}for(var f=a.getElementById(c).getElementsByTagName("div"),h=0;h<f.length;h++)f[h].getElementsByTagName("a")[0].addEventListener("click",b.objInit)},toolInit:function(a,d,f,h){"object"==typeof b.obj.drawTextObj&&!0==b.obj.drawTextObj.wmode&&vcan.main.canvas.getContext("2d");0<b.vcan.getStates("children").length&&
...
a.vcan.main.canvas.height);vcan.renderAll();d.save();d.beginPath();d.translate(c.mp.x,c.mp.y);d.drawImage(b,-3,-3,18,24);d.closePath();d.restore()},calcPsSentPackets:function(b){if(a.utility.chkValueInLocalStorage("orginalTeacherId"))return b=a.sentPackets-b,0>b&&(b=0),document.getElementById(a.sentPackDivPS).innerHTML=b,a.sentPackets},calcPsRecvdPackets:function(b){b=a.receivedPackets-b;0>b&&(b=0);document.getElementById(a.receivedPackDivPS).innerHTML=b;return a.receivedPackets},initStoredPacketsNumbers:function(){if(a.utility.chkValueInLocalStorage("orginalTeacherId")){if(localStorage.sentPackets){var b=
JSON.parse(localStorage.sentPackets);a.sentPackets=b;document.getElementById(a.sentPackDiv).innerHTML=b}localStorage.receivedPackets&&(a.receivedPackets=JSON.parse(localStorage.receivedPackets),document.getElementById(a.receivedPackDiv).innerHTML=a.receivedPackets)}},updateSentPackets:function(b){1==a.dataInfo&&a.utility.chkValueInLocalStorage("orginalTeacherId")&&(a.sentPackets+=JSON.stringify(b).length,document.getElementById(a.sentPackDiv).innerHTML=a.sentPackets)},assignRole:function(b){a.tool=
"";"move"==vcan.main.action&&a.utility.deActiveFrmDragDrop();if("undefined"!=typeof b){if(null!=localStorage.getItem("reclaim")){var c=document.getElementById(a.commandToolsWrapperId);c.parentNode.removeChild(c);localStorage.removeItem("reclaim")}g.whBoard.attachToolFunction(vcan.cmdWrapperDiv,!0);(a.hasOwnProperty("canvasDisable")||a.canvasDisable)&&a.utility.toolWrapperDisable();localStorage.setItem("teacherId",b);a.utility.makeCanvasEnable()}else{c=document.getElementById(a.commandToolsWrapperId);
if(null!=c)for(;c.hasChildNodes();)c.removeChild(c.lastChild);a.utility.makeCanvasDisable();"undefined"!=typeof localStorage.orginalTeacherId?(a.utility.createReclaimButton(c),localStorage.setItem("reclaim",!0)):null!=c&&c.parentNode.removeChild(c);localStorage.removeItem("teacherId")}},reclaimRole:function(){a.tool="";"move"==vcan.main.action&&a.utility.deActiveFrmDragDrop();a.utility.removeToolBox();g.whBoard.attachToolFunction(vcan.cmdWrapperDiv,!0);localStorage.teacherId=localStorage.orginalTeacherId;
"undefined"!=typeof localStorage.reclaim&&localStorage.removeItem("reclaim");a.utility.makeCanvasEnable()},connectionOff:function(){cthis.isInitiator=!1;cthis.pc=[];cthis.cn=0;cthis.isStarted=!1;cthis.byCommand=!0;io.disconnect()},connectionOn:function(){io.wsconnect()},dispQueuePacket:function(b){(null!=localStorage.getItem("teacherId")||null!=localStorage.getItem("orginalTeacherId")&&a.utility.chkValueInLocalStorage("reclaim"))&&a.utility.toolWrapperEnable();a.utility.isUserConnected(a.clientLen);
a.drawMode=!1;null!=localStorage.getItem("teacherId")&&a.user.connected&&a.utility.makeCanvasEnable();0<a.gObj.packQueue.length&&(g.whBoard.vcan.main.replayObjs=a.gObj.packQueue,a.gObj.packQueue=[],a.toolInit("t_replay","fromBrowser",!0,a.utility.dispQueuePacket))},updateRcvdInformation:function(a){var c=document.getElementById("rcvdMsgInfo");if(null!=c){var d="",f;for(f in a)d+=f+" : "+a[f]+" <br />";c.innerHTML=d}},makeCanvasDisable:function(){var a=vcan.main.canvas;a.style.position="relative";
a.style.pointerEvents="none"},makeCanvasEnable:function(){null==localStorage.getItem("teacherId")||a.hasOwnProperty("canvasDisable")&&a.canvasDisable||(vcan.main.canvas.style.pointerEvents="visible")},removeToolBox:function(){var a=document.getElementById(vcan.cmdWrapperDiv);a.parentNode.removeChild(a)},createReclaimButton:function(b){a.createDiv("t_reclaim","reclaim",b);document.getElementById("t_reclaim").getElementsByTagName("a")[0].addEventListener("click",a.objInit)},chkValueInLocalStorage:function(a){return null===
localStorage.getItem(a)?!1:localStorage[a]},uniqueArrOfObjsToStudent:function(){var b="";a.gObj.replayObjs=[];for(var c=0;c<vcan.main.replayObjs.length;c++)b=vcan.extend({},vcan.main.replayObjs[c]),a.gObj.replayObjs.push(b)},uniqueArrOfObjsToTeacher:function(){vcan.main.replayObjs=[];for(var b="",c=0;c<a.gObj.replayObjs.length;c++)b=vcan.extend({},a.gObj.replayObjs[c]),vcan.main.replayObjs.push(b)},makeDefaultValue:function(){a.gObj.myrepObj=[];a.gObj.replayObjs=[];a.gObj.rcvdPackId=0;a.gObj.displayedObjId=
0;a.gObj.packQueue=[];a.uid=0;var b=a.utility.chkValueInLocalStorage("teacherId"),c=a.utility.chkValueInLocalStorage("orginalTeacherId"),d=a.utility.chkValueInLocalStorage("wbrtcMsg"),f=a.utility.chkValueInLocalStorage("canvasDrwMsg"),h=a.utility.chkValueInLocalStorage("toolHeight");localStorage.clear();b&&localStorage.setItem("teacherId",b);c&&localStorage.setItem("orginalTeacherId",c);d&&localStorage.setItem("wbrtcMsg",d);f&&localStorage.setItem("canvasDrwMsg",f);h&&localStorage.setItem("toolHeight",
h);"undefined"!=typeof vcan.objTxt&&vcan.objTxt.removeTextNode();"undefined"!=typeof vcan.main.currentTransform&&(vcan.main.currentTransform="")},setOrginalTeacherContent:function(b){localStorage.setItem("teacherId",a.gObj.uid);g.whBoard.view.canvasDrawMsg("Canvas");localStorage.setItem("canvasDrwMsg",!0);a.utility.alreadyExistPacketContainer()||(a.createPacketContainer(),a.createPacketInfoContainer(),a.utility.initStoredPacketsNumbers());localStorage.setItem("orginalTeacherId",a.gObj.uid)},isSystemCompatible:function(){if(0<
g.whBoard.error.length)for(var b=0;b<g.whBoard.error.length;b++){var c=g.whBoard.error[b];c.hasOwnProperty("msg")&&a.view.displayMessage(c.msg,c.id,c.className)}},initDefaultInfo:function(b,c){var d=b.message.checkUser.e.clientLen,f=b.message.checkUser.e.newUser;"t"==c?null==localStorage.getItem("orginalTeacherId")&&(a.utility.setOrginalTeacherContent(b),g.whBoard.attachToolFunction(vcan.cmdWrapperDiv,!0)):"s"==c&&null==f&&(vcan.studentId=wbUser.id,null==localStorage.getItem("studentId")&&localStorage.setItem("studentId",
wbUser.id));1==d?(a.gObj.video.init(),a.gObj.video.isInitiator=!0,vcan.oneExecuted=!1):2<=d&&null==f&&(console.log("browser number "+d),2<d&&alert("there may be the problem because of user is more than 2"),a.utility.beforeSend({videoInt:!0}),a.utility.beforeSend({isChannelReady:!0,memberAdded:!0}),vcan.oneExecuted=!1,a.gObj.video.init())},checkWebRtcConnected:function(){return"undefined"==typeof cthis||!cthis.pc[0].hasOwnProperty("iceConnectionState")&&"undefined"==typeof cthis.pc[0].iceConnectionState?
!1:!0},createVirtualWindow:function(b){a.gObj.virtualWindow=!0;var c=document.createElement("div");a.utility.removeVirtualWindow("virtualWindow");c.setAttribute("id","virtualWindow");c.style.width=b.width-25+"px";if("undefined"!=typeof assignRoleAtStudent)var d=0;else d=parseInt(localStorage.getItem("toolHeight")),d=null!=d?d:0;null!=localStorage.getItem("orginalTeacherId")?c.style.height=b.height+d+"px":null!=localStorage.getItem("teacherId")?c.style.height=b.height+"px":c.style.height=b.height-
d+"px";b=document.getElementById("containerWb");b.insertBefore(c,b.firstChild)},removeVirtualWindow:function(b){b=document.getElementById(b);null!=b&&(a.gObj.virtualWindow=!1,b.parentNode.removeChild(b))},getWideValueAppliedByCss:function(a,c){var d=document.getElementById(a);if(null!=d){var f=g.getComputedStyle(d);if("undefined"!=typeof f.marginTop){var h=parseInt(f.marginTop.match(/\d+/));null==h&&(h=0)}return d.clientHeight+h}return!1},isNumber:function(a){return isNaN(+a)?!1:+a},setCommandToolHeights:function(a,
c){var d=document.getElementById("virtualWindow");if(null!=d){var f=parseInt(d.style.height.match(/\d+/));d.style.height="decrement"==c?f-parseInt(a)+"px":f+parseInt(a)+"px"}},setClass:function(a,c){var d=document.getElementById(a),f=d.classList,h="";0<f.length&&(h=2>h.length?f[0]+" ":f.join(" ")+" ");d.setAttribute("class",h+c)},isUserConnected:function(b){1<b&&localStorage.getItem("otherRole")&&(a.user.connected=!0)},isUserConnected_old:function(b){1<b&&(a.user.connected=!0)},setStyleUserConnetion:function(a,
...
console.log("Other Browser "+c+" "+b.fromUser.userid))}return a.gObj.uRole==c?!0:!1}}else{if("undefined"!=typeof existUser)return!0;b=JSON.parse(localStorage.getItem("otherRole"));if(null!=b){for(c=0;c<b.length;c++)if(a.gObj.uRole==b[c])return!0;return!1}}},existUserWithSameId:function(b){this.userIds.push(b.fromUser.userid);if(1<this.userIds.length&&a.utility.arrayContainsSameValue(this.userIds[0],this.userIds))return!0},makeUserAvailable:function(b){a.utility.isUserConnected(b);a.user.connected&&
(null==localStorage.getItem("repObjs")&&(a.utility.toolWrapperEnable(),null!=vcan.main.canvas&&a.utility.makeCanvasEnable()),a.utility.setStyleUserConnetion("coff","con"))},displayCanvas:function(){g.whBoard.attachToolFunction(vcan.cmdWrapperDiv);g.whBoard.init();a.utility.makeCanvasDisable();a.utility.toolWrapperDisable()},initAll:function(b){null!=localStorage.getItem("teacherId")&&a.utility.makeCanvasDisable();b=a.system.measureResoultion({width:g.outerWidth,height:g.innerHeight});var c=a.utility.getWideValueAppliedByCss("commandToolsWrapper");
!1!=c?a.utility.beforeSend({virtualWindow:{shareBrowserWidth:!0,browserRes:b,toolHeight:c,role:wbUser.role}}):a.utility.beforeSend({virtualWindow:{shareBrowserWidth:!0,browserRes:b,role:wbUser.role}})},alreadyExistToolBar:function(){var a=document.getElementById("t_rectangle");if(null!=a)return 8<=a.parentNode.getElementsByClassName("tool").length?!0:!1},alreadyExistPacketContainer:function(){var a=document.getElementById("packetContainer"),c=document.getElementById("informationCont");return 2<=a.getElementsByTagName("div").length||
1<=c.getElementsByTagName("div").length?!0:!1},toolWrapperDisable:function(){var a=document.getElementById("commandToolsWrapper");null!=a&&(a.style.pointerEvents="none")},toolWrapperEnable:function(){var a=document.getElementById("commandToolsWrapper");null!=a&&(a.style.pointerEvents="visible")},replayFromLocalStroage:function(){if("undefined"!==typeof Storage)if(null!=localStorage.getItem("repObjs")){var b=JSON.parse(localStorage.repObjs);a.vcan.main.replayObjs=b;a.utility.clearAll(!1,"dontClear");
a.gObj.replayObjs=a.gObj.replayObjs.concat(b);0<b.length&&(a.uid=b[b.length-1].uid,a.gObj.rcvdPackId=a.uid,a.toolInit("t_replay","fromBrowser",!0,a.utility.dispQueuePacket))}else a.utility.toolWrapperEnable()},shareVideoInformation:function(b,c){a.utility.isUserConnected(b.message.length);a.user.connected&&a.utility.setStyleUserConnetion("coff","con");"undefined"!=typeof vcan.teacher||c||a.utility.makeCanvasDisable();a.utility.initDefaultInfo(b,wbUser.role);var d=a.system.measureResoultion({width:g.outerWidth,
height:g.innerHeight}),f=a.utility.getWideValueAppliedByCss("commandToolsWrapper");!1!=f?a.utility.beforeSend({virtualWindow:{shareBrowserWidth:!0,browserRes:d,toolHeight:f}}):a.utility.beforeSend({virtualWindow:{shareBrowserWidth:!0,browserRes:d}})},setUserStatus:function(b,c){b||c?a.utility.setClass("vcanvas","teacher"):(a.utility.removeToolBox(),a.utility.setClass("vcanvas","student"))},crateCanvasDrawMesssage:function(){"undefined"!=typeof localStorage.teacherId&&null==localStorage.getItem("canvasDrwMsg")&&
(g.whBoard.view.canvasDrawMsg("Canvas"),g.whBoard.view.drawLabel("drawArea"),localStorage.canvasDrwMsg=!0)},removeOtherUserExist:function(a){"t"==a?null!=localStorage.getItem("studentId")&&localStorage.removeItem("studentId"):"s"==a&&null!=localStorage.getItem("orginalTeacherId")&&(localStorage.removeItem("orginalTeacherId"),null!=localStorage.getItem("teacherId")&&localStorage.removeItem("teacherId"))},canvasEnabelWhenRefresh:function(){null!=localStorage.getItem("teacherId")&&a.utility.makeCanvasEnable()},
arrayContainsSameValue:function(a,c){for(var d=0;d<c.length;d++)if(c[d]!==a)return!1;return!0},actionAfterRemovedUser:function(){a.utility.makeCanvasDisable();a.utility.setStyleUserConnetion("con","coff");a.utility.removeVirtualWindow("virtualWindow");a.user.connected=!1;localStorage.removeItem("otherRole");"undefined"!=typeof cthis&&(tempIsInitiaor=!0,cthis.isStarted&&cthis.handleRemoteHangup())},sendRequest:function(b,c){a.utility.beforeSend({reclaimRole:!0})},updateSentInformation:function(b,c){if(a.utility.chkValueInLocalStorage("orginalTeacherId")){var d=
JSON.parse(b),d="undefined"!=typeof c?d:d.repObj[0],f="",h;for(h in d)f+=h+" : "+d[h]+" <br />";document.getElementById("sentMsgInfo").innerHTML=f}},beforeSend:function(b){var c=JSON.stringify(b);b.hasOwnProperty("createArrow")?"undefined"==typeof optimizObj?optimizObj=optimization():optimizObj.sendPacketWithOptimization(c,io.sock.readyState,100):(a.sentPackets+=c.length,1==io.sock.readyState&&io.send(b),JSON.parse(c).hasOwnProperty("repObj")&&a.utility.updateSentInformation(c));localStorage.sentPackets=
a.sentPackets},checkCanvasHasParents:function(){for(var a=document.getElementById("vcanvas");"BODY"!=a.parentNode.tagName;)"vcanvas"!=a.id&&(a.style.margin="0",a.style.padding="0"),a=a.parentNode;"vcanvas"!=a.id&&(a.style.margin="0",a.style.padding="0")},lockCanvas:function(){if(g.earlierWidth!=g.innerWidth&&(a.canvasDisable=!0,a.utility.makeCanvasDisable(),a.utility.toolWrapperDisable(),null==document.getElementById("divForReloadMsg"))){var b=null!=localStorage.getItem("teacherId")?"msgForReload":
"msgStudentForReload";g.whBoard.view.displayMsgBox("divForReloadMsg",b);g.earlierWidth=g.innerWidth}},getElementRightOffSet:function(a){var c=20;null!=a.parentNode&&(a=a.parentNode,c=vcan.utility.getElementOffset(a),c=g.innerWidth-(a.clientWidth+(c.x-25)));return c}}})(window);(function(g){g.whBoard.lang.message={notSupportCanvas:'This browser is not supported for canvas. Please update the latest version of browser for more information about canvas visit the site <a href="http://en.wikipedia.org/wiki/Canvas_element/">Canvas</a>',notSupportGetUserMedia:'The browser is not supported for getUserMedia for webRtc. Please update your browser with latest version For more information about getUuserMedia visit <a href="http://dev.w3.org/2011/webrtc/editor/getusermedia.html">getUserMedia</a>',
notSupportPeerConnect:'The browser does not able to create Peer Connection object for WebRtc Please update your browser with latest version. For more information about WeRtc click on <a href="http://www.webrtc.org/">WebRtc</a>',notSupportWebSocket:'Your browser is not supported for WebSocket, please update your latest browser. For more information about WebSocket Please visit the site <a href="http://www.websocket.org/">WebSocket</a>.',notSupportWebRtc:'Your browser is not supported WebRTC. Please update your browser with latest version. For more information about WebRtc click on <a href="http://www.webrtc.org/">WebRtc</a>',
...
receviedPackets:"Received <br/><span>Pacekts</span>",total:"Total",dataDetails:"Data Details",sentMessageInfo:"Sent Message <br/><span>Information</span>",receivedMessageInfo:"Received Message <br/><span>information</span>",wbrtcMsgFireFox:'You can click on  "Share Selected Devices" to share your microphone and camera with other person',wbrtcMsgChrome:"You can click on deny button for not sharing your microphone and camera.while click on allow button to share your microphone and camera with Others.",
canvasDrawMsg:"You can click on any tool for draw antyhing with mouse click, mouse move and mouse up into Draw Area",clearAllWarnMessage:"Would you like to clear all what you have drawn, Once you clear the whiteboard, you will not get those drawn objects ever.",con:"connection on",cof:"connection off",askForConnect:"You will be able to perform this action after other user is connected",msgForReload:"You are not able to draw the object after resize the window. <br />  Please reload the page for drawing. ",
msgStudentForReload:"After resize the window there can be problem over the drawn objects, <br />  So please realod the page for better result ",reload:"Reload"}})(window);(function(g){g.whBoard.lang.getString=function(a){return g.whBoard.lang.message[a]}})(window);(function(g){whBoard=g.whBoard;whBoard.view.msgBoxClass="msgBox";whBoard.view.window={};whBoard.view.virtualWindow={};whBoard.view.displayMessage=function(a,b,c,d,f){a="undefined"==typeof f?whBoard.view.createMsgBox(a,b,c):whBoard.view.createMsgBox(a,b,c,f);b=document.getElementById("vcanvas");"undefined"!=typeof d?document.getElementById(d).appendChild(a):b.insertBefore(a,b.childNodes[0])};whBoard.view.customCreateElement=function(a,b,c){a=document.createElement(a);"undefined"!=typeof b&&(a.id=b);
...
500)};whBoard.view.virtualWindow.manupulation=function(a){var b=a.message.virtualWindow;if(b.hasOwnProperty("removeVirtualWindow"))a.fromUser.userid!=wbUser.id&&whBoard.utility.removeVirtualWindow("virtualWindow");else if(b.hasOwnProperty("resizeWindow"))myResolution=whBoard.system.measureResoultion({width:g.outerWidth,height:g.innerHeight}),a.fromUser.userid!=wbUser.id?(otherBrowser=a=b.resizeWindow,a.width<myResolution.width?whBoard.utility.createVirtualWindow(a):a.width==myResolution.width&&whBoard.utility.removeVirtualWindow("virtualWindow")):
"undefined"!=typeof otherBrowser&&(myResolution.width<otherBrowser.width?(whBoard.utility.beforeSend({virtualWindow:{resizeWindow:myResolution}}),whBoard.utility.removeVirtualWindow("virtualWindow")):myResolution.width>otherBrowser.width?(whBoard.utility.createVirtualWindow(otherBrowser),whBoard.utility.beforeSend({virtualWindow:{removeVirtualWindow:!0}})):myResolution.width==otherBrowser.width&&whBoard.utility.removeVirtualWindow("virtualWindow"));else if(b.hasOwnProperty("createVirtualWindow"))b.hasOwnProperty("toolHeight")&&
localStorage.setItem("toolHeight",b.toolHeight),a.fromUser.userid!=wbUser.id&&whBoard.utility.createVirtualWindow(b.createVirtualWindow);else if(b.hasOwnProperty("shareBrowserWidth")){b.hasOwnProperty("toolHeight")&&localStorage.setItem("toolHeight",b.toolHeight);if(null!=localStorage.getItem("teacherId")){var c=whBoard.utility.getWideValueAppliedByCss("commandToolsWrapper");localStorage.setItem("toolHeight",c)}a.fromUser.userid!=wbUser.id?(null!=localStorage.getItem("teacherId")&&whBoard.utility.makeCanvasEnable(),
otherBrowser=b.browserRes):myBrowser=whBoard.system.measureResoultion({width:g.outerWidth,height:g.innerHeight});"object"==typeof myBrowser&&"object"==typeof otherBrowser&&(myBrowser.width>otherBrowser.width?whBoard.gObj.virtualWindow||(whBoard.utility.createVirtualWindow(otherBrowser),whBoard.gObj.virtualWindow=!0):myBrowser.width<otherBrowser.width&&(whBoard.gObj.virtualWindow||(null!=localStorage.getItem("teacherId")?whBoard.utility.beforeSend({virtualWindow:{createVirtualWindow:myBrowser,toolHeight:c}}):
whBoard.utility.beforeSend({virtualWindow:{createVirtualWindow:myBrowser}}))))}}})(window);(function(g){whBoard.system.wbRtc={};whBoard.system.wbRtc.className="webrtcCont";whBoard.system.mybrowser={};whBoard.system.mybrowser.detection=function(){var a=navigator.userAgent,b,f=a.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*([\d\.]+)/i)||[];if(/trident/i.test(f[1]))return b=/\brv[ :]+(\d+(\.\d+)?)/g.exec(a)||[],"IE "+(b[1]||"");f=f[2]?[f[1],f[2]]:[navigator.appName,navigator.appVersion,"-?"];null!=(b=a.match(/version\/([\.\d]+)/i))&&(f[2]=b[1]);return f};whBoard.system.isCanvasSupport=
...
b.appendChild(c);c=a.createPacketContDiv("receivedMsgInfoContainer");label=document.createElement("label");label.innerHTML=a.lang.getString("receivedMessageInfo");c.appendChild(label);d=a.createPacketContDiv("rcvdMsgInfo");c.appendChild(d);b.appendChild(c)}})(window);(function(g){var a=g.whBoard;a.draw_object=function(b,c,d){d.started=!1;var f,h,k=[];d.mousedown=function(c,f){c.detail.hasOwnProperty("cevent")&&(c.clientX=c.detail.cevent.x+a.vcan.main.offset.x,c.clientY=c.detail.cevent.y+a.vcan.main.offset.y,c.x=c.detail.cevent.x+a.vcan.main.offset.x,c.y=c.detail.cevent.x+a.vcan.main.offset.y,c.pageX=c.detail.cevent.x+a.vcan.main.offset.x,c.pageY=c.detail.cevent.y+a.vcan.main.offset.y,c.currX=c.detail.cevent.x,c.currY=c.detail.cevent.y);var h=a.vcan;lastmousemovetime=
null;d.startPosX=c.currX;d.startPosY=c.currY;if("create"==h.getStates("action")){var k=(new Date).getTime();if("text"!=b&&"t_clearall"!=a.tool.cmd){var g=h.getStates("currentTransform");if(""==g||null==g)c.detail.hasOwnProperty("cevent")||h.calculatePackets(k,"d",d.startPosX,d.startPosY),d.started=!0}else a.obj.drawTextObj.muser=!1,c.detail.hasOwnProperty("cevent")||"t_clearall"==a.tool.cmd?a.obj.drawTextObj.muser=!0:a.utility.clickOutSidebox(a.obj.drawTextObj.textWriteMode)&&h.calculatePackets(k,
...
typeof lastmousemovetime&&null!=lastmousemovetime||(lastmousemovetime=(new Date).getTime(),vcan.calculatePackets(lastmousemovetime,"m",c.currX,c.currY));var m=(new Date).getTime(),m=vcan.makeStackObj(m,"m",c.currX,c.currY);k.push(m);if("undefined"==typeof g&&(presentmousemovetime=(new Date).getTime(),2E3<=presentmousemovetime-lastmousemovetime)){for(m=0;m<k.length;m++)a.uid++,k[m].uid=a.uid,vcan.main.replayObjs.push(k[m]);a.utility.beforeSend({repObj:k});a.utility.updateSentPackets(k);localStorage.repObjs=
JSON.stringify(vcan.main.replayObjs);k=[];lastmousemovetime=(new Date).getTime()}}}else{f=c.currX;h=c.currY;""!=a.prvObj&&a.canvas.removeObject(a.prvObj);var m=a.makeobj(d.startPosX,d.startPosY,f,h,b),l=a.canvas.readyObject(m);a.canvas.addObject(l);l.coreObj.usrCurrAction="create";m=(new Date).getTime();if("undefined"==typeof lastmousemovetime||null==lastmousemovetime)lastmousemovetime=(new Date).getTime(),c.detail.hasOwnProperty("cevent")||"text"==b||"t_clearall"==a.tool.cmd||vcan.calculatePackets(m,
"m",f,h);presentmousemovetime=(new Date).getTime();2E3<=presentmousemovetime-lastmousemovetime&&(c.detail.hasOwnProperty("cevent")||"text"==b||"t_clearall"==a.tool.cmd||vcan.calculatePackets(m,"m",f,h),lastmousemovetime=(new Date).getTime());a.prvObj=l.coreObj}else"move"==a.vcan.main.action&&(""!=vcan.main.currentTransform&&null!=vcan.main.currentTransform||"move"!=a.vcan.main.action)||a.utility.beforeSend({createArrow:!0,x:c.currX,y:c.currY})};d.mouseup=function(c,g){c.detail.hasOwnProperty("cevent")&&
(c.clientX=c.detail.cevent.x+a.vcan.main.offset.x,c.clientY=c.detail.cevent.y+a.vcan.main.offset.y,c.x=c.detail.cevent.x+a.vcan.main.offset.x,c.y=c.detail.cevent.x+a.vcan.main.offset.y,c.pageX=c.detail.cevent.x+a.vcan.main.offset.x,c.pageY=c.detail.cevent.y+a.vcan.main.offset.y,c.currX=c.detail.cevent.x,c.currY=c.detail.cevent.y);f=c.currX;h=c.currY;lastmousemovetime=null;if(d.started&&"text"!=b){d.mousemove(c,"up");if(!c.detail.hasOwnProperty("cevent")&&"freeDrawing"!=b){var m=(new Date).getTime();
vcan.calculatePackets(m,"u",f,h)}if(void 0!=a.obj.freeDrawObj&&!0==a.obj.freeDrawObj.freesvg&&(a.obj.freeDrawObj.fdObj.isCurrentlyDrawing&&a.obj.freeDrawObj.finalizeDraw(c),!c.detail.hasOwnProperty("cevent")&&0<k.length)){m=(new Date).getTime();m=vcan.makeStackObj(m,"u",f,h);k.push(m);for(m=0;m<k.length;m++)a.uid++,k[m].uid=a.uid,vcan.main.replayObjs.push(k[m]);a.utility.beforeSend({repObj:k});localStorage.repObjs=JSON.stringify(vcan.main.replayObjs);a.utility.updateSentPackets(k);k=[]}""!=a.prvObj&&
(a.prvObj="");a.prvObj="";d.started=!1}a.vcan.wb.sentPack&&(a.vcan.wb.sentPack=!1)}}})(window);(function(g){g.whBoard.makeobj=function(a,b,c,d,f){var h={mp:{}};h.mp.x=c;h.mp.y=d;if(a>c){var k=c,g=d;c=a;d=b;a=k;b=g;h.dRoad="b2t"}else h.dRoad="t2b";h.type=f;if("rectangle"==f||"triangle"==f)h.sx=a,h.sy=b,h.ex=c,h.ey=d;if("line"==f)h.lineColor="#424240",h.lineWidth=3,h.start={x:a,y:b},h.end={x:c,y:d};else if("rectangle"==f)h.width=c-a,h.height=d-b,h.borderColor="#424240",h.lineWidth=3;else if("oval"==f)h.lineWidth=3,h.borderColor="#424240",h.width=c-a,h.height=d-b,h.x=c-h.width/2,h.y=d-h.height/
2,h.rx=h.width/2,h.ry=h.height/2;else if("triangle"==f)h.lineWidth=3,h.borderColor="#424240";else if("text"==f)return;return h}})(window);(function(g){var a=g.vcan;a.utility={canvasCalcOffset:function(b){var c=a.main;b=document.getElementById(b);b=a.utility.getElementOffset(b);return c.offset=b},getElementOffset:function(a){var c=0,d=0;do c+=a.offsetTop||0,d+=a.offsetLeft||0,a=a.offsetParent;while(a);return{x:d,y:c}},setVal:function(b,c,d){("scaleX"===c||"scaleY"===c)&&d<b.MIN_SCALE_LIMIT&&(d=b.MIN_SCALE_LIMIT);if("object"==typeof c)for(var f in c)a.utility.setVal(b,f,c[f]);else"angle"===c?a.utility.setAngle(b,d):b[c]=d;return b},getVal:function(a,
...
this.currentWidth);this.theta=this.theta;var d=Math.cos(this.angle+this.theta)*this.hypotenuse,c=Math.sin(this.angle+this.theta)*this.hypotenuse,h=this.theta,k=Math.sin(h),h=Math.cos(h),d={x:this.x-d,y:this.y-c},c={x:d.x+this.currentWidth*h,y:d.y+this.currentWidth*k},g={x:d.x-this.currentHeight*k,y:d.y+this.currentHeight*h};this.oCoords={tl:d,tr:c,br:{x:c.x-this.currentHeight*k,y:c.y+this.currentHeight*h},bl:g,ml:{x:d.x-this.currentHeight/2*k,y:d.y+this.currentHeight/2*h},mt:{x:d.x+this.currentWidth/
2*h,y:d.y+this.currentWidth/2*k},mr:{x:c.x-this.currentHeight/2*k,y:c.y+this.currentHeight/2*h},mb:{x:g.x+this.currentWidth/2*h,y:g.y+this.currentWidth/2*k},mtr:{x:d.x+this.currentWidth/2*h,y:d.y+this.currentWidth/2*k}};this.setCornerCoords();b.findTargetCorner=function(d){var c=a.main.offset;if(!this.hasControls)return!1;var f=a.utility.actualPointer(d);d=f.x-c.x;var c=f.y-c.y,h;for(h in this.oCoords){if("mtr"===h&&!this.hasRotatingPoint)break;f=a.virtual_box.getImageLines(this.oCoords[h].corner,
h);f=a.virtual_box.findCrossPoints(d,c,f);if(1==f%2&&0!=f)return b.corner=h}return!1};return this};b.drawBorders=function(a){if(this.hasBorders){var b=this.padding,c=2*b;a.save();a.globalAlpha=this.isMoving?this.borderOpacityWhenMoving:1;a.strokeStyle=this.borderColor;var k=1/(this.scaleX<this.MIN_SCALE_LIMIT?this.MIN_SCALE_LIMIT:this.scaleX),g=1/(this.scaleY<this.MIN_SCALE_LIMIT?this.MIN_SCALE_LIMIT:this.scaleY);a.lineWidth=1/this.borderScaleFactor;a.scale(k,g);k=this.getWidth();g=this.getHeight();
a.strokeRect(~~(-(k/2)-b)+.5,~~(-(g/2)-b)+.5,~~(k+c),~~(g+c));!this.hasRotatingPoint||this.hideCorners||this.lockRotation||(b=(this.flipY?g:-g)/2,a.beginPath(),a.moveTo(0,b),a.lineTo(0,b+(this.flipY?this.rotatingPointOffset:-this.rotatingPointOffset)),a.closePath(),a.stroke());a.restore();return this}};b.drawCorners=function(a){if(this.hasControls){var b=this.cornersize,c=b/2,k=this.padding,g=-(this.width/2),n=-(this.height/2),m=b/this.scaleX,l=b/this.scaleY,r=(k+c)/this.scaleY,s=(k+c)/this.scaleX,
q=(k+c-b)/this.scaleX,c=(k+c-b)/this.scaleY,k=this.height;a.save();a.fillStyle=this.cornerColor;a.fillRect(g-s,n-r,m,l);b=g+this.width-s;a.fillRect(b,n-r,m,l);a.fillRect(g-s,n+k+c,m,l);b=g+this.width+q;a.fillRect(b,n+k+c,m,l);b=g+this.width/2-s;a.fillRect(b,n-r,m,l);b=g+this.width/2-s;a.fillRect(b,n+k+c,m,l);b=g+this.width+q;a.fillRect(b,n+k/2-r,m,l);a.fillRect(g-s,n+k/2-r,m,l);this.hasRotatingPoint&&(b=g+this.width/2-s,g=this.flipY?n+k+this.rotatingPointOffset/this.scaleY-l/2:n-this.rotatingPointOffset/
this.scaleY-l/2,a.fillRect(b,g,m,l));a.restore();return this}};b.setActive=function(a){this.active=!!a;return this};b.dragDrop=function(a){this.draggable=void 0!=a?a:!1};b.setCornerCoords=function(){var b=this.oCoords,c=a.utility.degreesToRadians(45-this.getAngle()),h=Math.sqrt(2*Math.pow(this.cornersize,2))/2,k=h*Math.cos(c),c=h*Math.sin(c),h=Math.sin(this.theta),g=Math.cos(this.theta);b.tl.corner={tl:{x:b.tl.x-c,y:b.tl.y-k},tr:{x:b.tl.x+k,y:b.tl.y-c},bl:{x:b.tl.x-k,y:b.tl.y+c},br:{x:b.tl.x+c,y:b.tl.y+
...
{tl:{x:b.mt.x-c,y:b.mt.y-k},tr:{x:b.mt.x+k,y:b.mt.y-c},bl:{x:b.mt.x-k,y:b.mt.y+c},br:{x:b.mt.x+c,y:b.mt.y+k}};b.mr.corner={tl:{x:b.mr.x-c,y:b.mr.y-k},tr:{x:b.mr.x+k,y:b.mr.y-c},bl:{x:b.mr.x-k,y:b.mr.y+c},br:{x:b.mr.x+c,y:b.mr.y+k}};b.mb.corner={tl:{x:b.mb.x-c,y:b.mb.y-k},tr:{x:b.mb.x+k,y:b.mb.y-c},bl:{x:b.mb.x-k,y:b.mb.y+c},br:{x:b.mb.x+c,y:b.mb.y+k}};b.mtr.corner={tl:{x:b.mtr.x-c+h*this.rotatingPointOffset,y:b.mtr.y-k-g*this.rotatingPointOffset},tr:{x:b.mtr.x+k+h*this.rotatingPointOffset,y:b.mtr.y-
c-g*this.rotatingPointOffset},bl:{x:b.mtr.x-k+h*this.rotatingPointOffset,y:b.mtr.y+c-g*this.rotatingPointOffset},br:{x:b.mtr.x+c+h*this.rotatingPointOffset,y:b.mtr.y+k-g*this.rotatingPointOffset}}};b.getAngle=function(){return 180*this.theta/Math.PI};b.setZindex=function(){for(var b=0;b<a.main.children.length;b++)if(this.id==a.main.children[b].id){var c=a.main.children[b];a.main.children.splice(b,1);break}return" "!=c?(a.main.children.push(c),!0):!1};b.setupCurrentTransform=function(b){var c=a.main,
h="drag",k=a.utility.actualPointer(b);if(b=this.findTargetCorner(b))h="ml"===b||"mr"===b?"scaleX":"mt"===b||"mb"===b?"scaleY":"mtr"===b?"rotate":this.hasRotatingPoint?"scale":"rotate";c.currentTransform={target:this,action:h,scaleX:this.scaleX,scaleY:this.scaleY,offsetX:k.x-this.x,offsetY:k.y-this.y,ex:k.x,ey:k.y,x:this.x,y:this.y,theta:this.theta,width:this.width*this.scaleX};c.currentTransform.original={x:this.x,y:this.y}};"text"!=b.type&&b.setCoords();return a.makeDispObject(b)}}})(window);(function(g){var a=g.vcan;a.events=function(){return{bind:function(a,c,d){a["on"+c]=d},findTarget:function(b){for(var c,d=a.main.children,f=d.length;f--;)if(d[f]&&a.virtual_box.containsPoint(b,d[f])){c=d[f];break}if(c&&c.selectable)return c}}}})(window);(function(g){var a=g.vcan;a.virtual_box={findCrossPoints:function(a,c,d){var f,h,k,g=0,n;for(n in d)if(k=d[n],!(k.o.y<c&&k.d.y<c||k.o.y>=c&&k.d.y>=c)&&(k.o.x==k.d.x&&k.o.x>=a?f=k.o.x:(f=(k.d.y-k.o.y)/(k.d.x-k.o.x),h=c-0*a,k=k.o.y-f*k.o.x,f=-(h-k)/(0-f)),f>=a&&(g+=1),2==g))break;return g},getImageLines:function(a,c){return{topline:{o:a.tl,d:a.tr},rightline:{o:a.tr,d:a.br},bottomline:{o:a.br,d:a.bl},leftline:{o:a.bl,d:a.tl}}},containsPoint:function(b,c){var d=a.utility.getReltivePoint(b),f=d.x,d=d.y,
h=a.virtual_box.getImageLines(c.oCoords);return(f=a.virtual_box.findCrossPoints(f,d,h))&&1===f%2||c.findTargetCorner(b)?!0:!1}}})(window);(function(g){var a=g.vcan;a.interact={rotateObject:function(b,c){var d=a.main.currentTransform,f=a.main.offset;if(!d.target.lockRotation){var h=Math.atan2(d.ey-d.y-f.y,d.ex-d.x-f.x),f=Math.atan2(c-d.y-f.y,b-d.x-f.x);a.utility.setVal(d.target,"theta",f-h+d.theta)}},scaleObject:function(b,c,d){var f=a.main.currentTransform,h=a.main.offset;target=f.target;if(!target.lockScalingX||!target.lockScalingY){var k=Math.sqrt(Math.pow(f.ey-f.y-h.y,2)+Math.pow(f.ex-f.x-h.x,2));b=Math.sqrt(Math.pow(c-f.y-h.y,2)+
Math.pow(b-f.x-h.x,2));target.scaling=!0;d?"x"!==d||target.lockUniScaling?"y"!==d||target.lockUniScaling||target.lockScalingY||a.utility.setVal(target,"scaleY",f.scaleY*b/k):target.lockScalingX||a.utility.setVal(target,"scaleX",f.scaleX*b/k):(target.lockScalingX||a.utility.setVal(target,"scaleX",f.scaleX*b/k),target.lockScalingY||a.utility.setVal(target,"scaleY",f.scaleY*b/k))}},translateObject:function(b,c){var d=a.main.currentTransform.target;d.lockMovementX||a.utility.setVal(d,"x",b-a.main.currentTransform.offsetX);
d.lockMovementY||a.utility.setVal(d,"y",c-a.main.currentTransform.offsetY);return d}}})(window);(function(g){g.vcan.rectangle=function(){return{type:"rectangle",init:function(a){void 0==a.x&&(a.x=a.ex-a.width/2);void 0==a.y&&(a.y=a.ey-a.height/2);return a},draw:function(a,b,c){c=-b.width/2;var d=-b.height/2,f=b.width,h=b.height;a.beginPath();a.strokeStyle=void 0!=b.borderColor?'"'+b.strokeColor+'"':"#000";a.moveTo(c,d);a.stroke();a.lineTo(c+f,d);a.stroke();a.lineTo(c+f,d+h);a.stroke();a.lineTo(c,d+h);a.stroke();a.lineTo(c,d);a.fillStyle=void 0!=b.fillColor?b.fillColor:" ";a.closePath();a.stroke();
...
this.contextTop.closePath();this.isCurrentlyDrawing=!1;var h=this.utility.min(this.freeDrawingXPoints);d=this.utility.min(this.freeDrawingYPoints);var k=this.utility.max(this.freeDrawingXPoints);f=this.utility.max(this.freeDrawingYPoints);var g=[],n,m,l,r=this.freeDrawingXPoints,s=this.freeDrawingYPoints,q=this.mdTime;g.push("M ",r[0]-h," ",s[0]-d," ",q[0]," ");for(var t=1;n=r[t],m=s[t],l=q[t];t++)g.push("L ",n-h," ",m-d," ",l," ");g=g.join("");if("M 0 0 L 0 0 "!==g)return n=this.freeDrawingXPoints[this.freeDrawingXPoints.length-
1],m=this.freeDrawingYPoints[this.freeDrawingYPoints.length-1],l=new a.Path(g,this),l.mp={},l.mp.x=n,l.mp.y=m,l.init(g),l=c.readyObject(l),l.coreObj.type="freeDrawing",l=l.coreObj,l.fill=null,l.stroke=this.freeDrawingColor,l.strokeWidth=this.freeDrawingLineWidth,l.minX=h,l.minY=d,this.utility.objAdd(l),c=a.utility.setVal(l,"x",h+(k-h)/2),c=a.utility.setVal(c,"y",d+(f-d)/2),c.setCoords(),"object"==typeof b&&this.contextTop.restore(),a.renderAll(this.contextTop),c},utility:{max:function(a,b){if(a&&
...
_render:function(a){var b,c=0;b=0;for(var k=-(this.width/2),g=-(this.height/2),n=0,m=this.path.length;n<m;++n)switch(b=this.path[n],b[0]){case "L":c=b[1];b=b[2];a.lineTo(c+k,b+g);break;case "M":c=b[1],b=b[2],a.moveTo(c+k,b+g)}},replay_render:function(a,b,c,k){var g=y=0;switch(b[0]){case "L":g+=b[1];y+=b[2];a.lineTo(g+c,y+k);a.stroke();break;case "M":g+=b[1],y+=b[2],a.moveTo(g+c,y+k)}},render:function(b,c){b.save();var h=this.transformMatrix;h&&b.transform(h[0],h[1],h[2],h[3],h[4],h[5]);c||a.transform(b,
this);this.overlayFill?b.fillStyle=this.overlayFill:this.fill&&(b.fillStyle=this.fill);this.stroke&&(b.strokeStyle=this.stroke);b.beginPath();this._render(b);this.fill&&b.fill();this.stroke&&(b.strokeStyle=this.stroke,b.lineWidth=this.strokeWidth,b.lineCap=b.lineJoin="round",b.stroke());!c&&this.active&&(this.drawBorders(b),this.hideCorners||this.drawCorners(b));b.restore()},_parsePath:function(){for(var a=[],b,c,g,p=0,n,m=this.path.length;p<m;p++){var l=/\d+\s$/.exec(this.path[p]);null!=l&&(b=this.path[p].indexOf(l),
this.path[p]=this.path[p].substring(0,b));b=this.path[p];c=b.slice(1).trim().replace(/(\d)-/g,"$1###-").split(/\s|,|###/);b=[b.charAt(0)];n=0;for(var r=c.length;n<r;n++)g=parseFloat(c[n]),isNaN(g)||b.push(g);c=b[0].toLowerCase();c=this.commandLengths[c];if(b.length-1>c)for(g=1,n=b.length;g<n;g+=c)null==l?a.push([b[0]].concat(b.slice(g,g+c))):(a.push([b[0]].concat(b.slice(g,g+c))),b.push(l),a.push(b));else null!=l&&b.push(l[0]),a.push(b)}return a},_parseDimensions:function(){var a=[],b=[],h,g,p=!1,
n,m;this.path.forEach(function(c,l){if(" "!=c[3])var q=c.splice(3);"H"!==c[0]&&(h=0===l?this.utility.getX(c):this.utility.getX(this.path[l-1]));"V"!==c[0]&&(g=0===l?this.utility.getY(c):this.utility.getY(this.path[l-1]));c[0]===c[0].toLowerCase()&&(p=!0);n=p?h+this.utility.getX(c):"V"===c[0]?h:this.utility.getX(c);m=p?g+this.utility.getY(c):"H"===c[0]?g:this.utility.getY(c);var t=parseInt(n,10);isNaN(t)||a.push(t);t=parseInt(m,10);isNaN(t)||b.push(t);void 0!=q&&1<=q.length&&c.push(q[0])},this);var l=
...
else if(c.currentTransform.target.draggable){var h=a.extend({},a.main.currentTransform.target);(new Date).getTime();!0==c.currentTransform.target.downObj&&(a.main.dragMode=!0,c.currentTransform.target.downObj=!1);h=a.interact.translateObject(f,d);b.detail.hasOwnProperty("cevent")||a.doOptiMize(b);h.setActive(!0);h.setCoords()}if("scaleX"===c.currentTransform.action||"scaleY"===c.currentTransform.action||"rotate"===c.currentTransform.action&&!c.currentTransform.target.hasRotatingPoint)a.main.scaleMode=
...
c,d,f){var h=a.vcan;if(""!=this.prevTextObj){if(!a.utility.IsObjEmpty(b))for(var g=0;g<h.main.children.length;g++)if(b.id==h.main.children[g].id){h.main.children.splice(g,1);this.currObject="";break}a.utility.IsObjEmpty(b)?"undefined"!=typeof f?this.finalizeText(d,this.prevTextObj,void 0,f):this.finalizeText(d,this.prevTextObj):"undefined"!=typeof f?this.finalizeText(d,this.prevTextObj,c,f):this.finalizeText(d,this.prevTextObj,c);a.obj.drawTextObj.wmode=!1}},finalizeText:function(b,c,d,f){d=a.vcan;
var h=document.getElementById(c.id),g="";if("undefined"==typeof f){f=h.getElementsByTagName("textarea");for(var p=0;p<f.length;p++)if(f[p].id==h.id+"textarea"){g=f[p].value;break}}else g=f;b.font="20px Times New Roman";var n=0,m=g.split(/\r?\n/),n=b.measureText(m[0]).width;f=15;for(p=1;p<m.length;p++){f+=15;var l=b.measureText(m[p]).width;l>n&&(n=l)}n/=2;p=(new Date).getTime();b={type:"text",text:g,x:c.measure.x+n,y:c.measure.y+f,fontSize:20,fontWidth:b.measureText(g).width,mp:{x:c.measure.x,y:c.measure.y}};
!1==a.obj.drawTextObj.muser&&(g={mt:p,ac:"d",x:this.startPosX,y:this.startPosY,mtext:b.text},a.uid++,g.uid=a.uid,d.main.replayObjs.push(g),localStorage.repObjs=JSON.stringify(d.main.replayObjs),a.utility.beforeSend({repObj:[g]}),a.utility.updateSentPackets(g));b=a.canvas.readyObject(b);a.canvas.addObject(b);d.main.children[d.main.children.length-1].mt=p;h.parentNode.removeChild(c);d.renderAll();0<a.sentPackets&&(document.getElementById(a.sentPackDiv).innerHTML=a.sentPackets)}}}})(window);(function(g){g.onload=function(){var a=g.whBoard;a.keyBoard={prvTool:"",skey:!1,triggerActiveAll:function(b){b.shiftKey&&(console.log("what happend mere bhai"),a.keyBoard.skey=!0,a.keyBoard.prvTool=a.tool.cmd,a.toolInit("t_activeall"),b={cmd:"t_activeall",mt:(new Date).getTime()},vcan.main.replayObjs.push(b),a.utility.beforeSend({repObj:[b]}),a.vcan.main.action="move")},triggerdeActiveAll:function(b){a.keyBoard.skey&&(console.log("what happend mere bhai ddd"),b=(new Date).getTime(),a.utility.deActiveFrmDragDrop(),
...
!1;if(b){var d=c=0,f=a.get.px(a._style(b,"marginLeft")),g=a.get.px(a._style(b,"marginTop")),k=a.get.px(a._style(b,"left")),p=a.get.px(a._style(b,"top")),n=b.offsetLeft,m=b.offsetTop;""!==k&&""!==p&&(n>k+f||m>p+g)&&(c=a.get.px(a._style(b.parentNode,"borderLeft")),d=a.get.px(a._style(b.parentNode,"borderTop")));c={left:0<c?n-c:n,top:0<d?m-d:m}}return c}},parent:function(a,c,d){if(a){for(var f=0;5>f;f++)if(c.test(a.className))if(d)a=a.parentNode;else break;else if(d)break;else a=a.parentNode;return a}},
db:{id:function(b){var c,d={found:!1,data:""},f=a.divList.length;if(b)for(c=0;c<f;c++)if("zIndex"===b)"auto"!==a.divList[c][3]&&a.divList[c][3]>d.data&&(d.found=!0,d.data=a.divList[c][3]);else if(a.divList[c][0]===b){d.found=!0;d.data=c;break}return d}},hit:function(b,c,d){var f=!1;if(b&&c){var g=a.get.offset.absolute(b),k=b.clientWidth,p=b.clientHeight;b=a.get.offset.absolute(c);var p=g.top+p,n=b.top+c.clientHeight,k=g.left+k;c=b.left+c.clientWidth;switch(d){case "fit":case "center":if(g.left>b.left&&
g.left<c&&(g.top>b.top&&g.top<n||p>b.top&&p<n)||k>b.left&&k<c&&(g.top>b.top&&g.top<n||p>b.top&&p<n))f=!0;break;default:g.left>b.left&&g.left<c&&k<c&&g.top>b.top&&g.top<n&&p<n&&(f=!0)}}return f}},on:{move:function(b){if(a.cache.obj){a.get.mouse(b);a.get.db.id("zIndex").found&&a._style(a.cache.obj,"zIndex",a.get.db.id("zIndex").data+1);if(a.cache.modus)switch(a.api.alter&&a.api.alter(),a.cache.modus){case "br":case "tr":case "tl":case "bl":a.resize();break;case "move":case "moveparent":a.move()}a.cache.ie&&
...
a.db(9)&&"move_resize"!==a.db(9)||a.set.visible(!1)}a.cache.elem===a.cache.last.elem||!a.cache.last.elem&&0!==a.cache.last.elem||a.divList[a.cache.last.elem][10]&&a.on.resize(a.cache.last.obj,!1);"active"!==a.db(10)&&"focus"!==a.db(10)||a.on.resize(a.cache.obj,!0)}}}},settings:{save:function(){if(a.divList){var b,c=a.divList.length;for(b=0;b<c;b++)if(!1!==a.divList[b][12]&&a.divList[b][0].id){var d="",f="",g="",k="";switch(a.divList[b][12]){case "position":d=a.divList[b][4];f=a.divList[b][5];break;
case "size":g=a.divList[b][13];k=a.divList[b][14];break;case "position_size":d=a.divList[b][4],f=a.divList[b][5],g=a.divList[b][13],k=a.divList[b][14]}if(("NaN"!==Number(d)&&"NaN"!==Number(f)||0<g&&0<k)&&navigator.cookieEnabled){var p=new Date;p.setSeconds((new Date).setTime(p.getSeconds())+Number(a.config.cookie.expire));document.cookie="ByRei_dynDiv_"+a.divList[b][0].id+"="+d+"_"+f+"_"+g+"_"+k+"; expires="+p.toGMTString()}}}},load:function(a){var c=!1;if(navigator.cookieEnabled&&a){var d=document.cookie;
/; /.test(d)?d=d.split("; "):/, /.test(d)&&(d=d.split(", "));if(d){var f,g=d.length;for(f=0;f<g;f++)if(d[f]&&/ByRei_dynDiv/.test(d[f])){var k=/ByRei_dynDiv_(\w+)=(\d*)\_(\d*)\_(\d*)\_(\d*)/.exec(d[f]);if("undefined"!==typeof k&&null!==k&&"undefined"!==typeof k[1]&&k[1]===a){c={left:k[2],top:k[3],width:k[4],height:k[5]};break}}}}return c}},set:{visible:function(b){var c,d=a.divList.length;for(c=0;c<d;c++)a.divList[c]!==a.divList[a.cache.elem]&&a._style(a.divList[c][0],"visibility",b?"visible":"hidden")},
left:function(b,c){a._style(b,"left",c+"px")},top:function(b,c){a._style(b,"top",c+"px")},width:function(b,c){a._style(b,"width",c+"px")},height:function(b,c){a._style(b,"height",c+"px")}},resize:function(){if(a.cache.obj&&a.cache.modus){var b=0,c=0,d=a.db(4),f=a.db(5),g=a.db(11),k=!1,p=a.cache.pos.left-a.cache.init.pos.left||0,n=a.cache.pos.top-a.cache.init.pos.top||0;if(g){switch(a.cache.modus){case "br":case "tl":p=n*g;break;case "bl":case "tr":p=n*g*-1}if(a.cache.last.mouse.left===a.cache.pos.left||
a.cache.last.mouse.top===a.cache.pos.top)k=!0;a.cache.last.mouse.left=a.cache.pos.left;a.cache.last.mouse.top=a.cache.pos.top}switch(a.cache.modus){case "br":case "tr":b=a.cache.init.width+p;break;case "tl":case "bl":b=a.cache.init.width-p}switch(a.cache.modus){case "br":case "bl":c=a.cache.init.height+n;break;case "tr":case "tl":c=a.cache.init.height-n}switch(a.cache.modus){case "tl":f=a.db(5)+n;d=a.db(4)+p;break;case "tr":f=a.db(5)+n;break;case "bl":d=a.db(4)+p}if(a.db(1)){p=a.cache.pos.left-a.cache.init.pos.left;
n=a.cache.pos.top-a.cache.init.pos.top;switch(a.cache.modus){case "tl":case "bl":p<a.limit.min.left&&(g||(b=a.cache.init.width-a.limit.min.left,d=a.db(4)+a.limit.min.left),k=!0);break;case "tr":case "br":p>a.limit.max.left&&(g||(b=a.cache.init.width+a.limit.max.left),k=!0)}switch(a.cache.modus){case "tl":case "tr":n<a.limit.min.top&&(g||(c=a.cache.init.height-a.limit.min.top,f=a.db(5)+a.limit.min.top),k=!0);break;case "bl":case "br":n>a.limit.max.top&&(g||(c=a.cache.init.height+a.limit.max.top),k=
!0)}}g?!k&&20<b&&20<c&&(a.set.width(a.cache.obj,b),a.set.height(a.cache.obj,c),a.set.left(a.cache.obj,d),a.set.top(a.cache.obj,f)):(20<b&&(a.set.width(a.cache.obj,b),a.set.left(a.cache.obj,d)),20<c&&(a.set.height(a.cache.obj,c),a.set.top(a.cache.obj,f)))}},move:function(){if(a.cache.obj){var b=a.cache.pos.left-(a.cache.init.pos.left-a.db(4)),c=a.cache.pos.top-(a.cache.init.pos.top-a.db(5));if(a.db(1)){var d=a.cache.pos.left-a.cache.init.pos.left,f=a.cache.pos.top-a.cache.init.pos.top;d<a.limit.min.left?
b=a.db(4)+a.limit.min.left:d>a.limit.max.left&&(b=a.db(4)+a.limit.max.left);f<a.limit.min.top?c=a.db(5)+a.limit.min.top:f>a.limit.max.top&&(c=a.db(5)+a.limit.max.top)}isNaN(b)||a.set.left(a.cache.obj,b);isNaN(c)||a.set.top(a.cache.obj,c)}},db:function(b,c){var d=!1;0<=a.cache.elem&&a.divList[a.cache.elem]&&"undefined"!==typeof a.divList[a.cache.elem][b]&&("undefined"!==typeof c?(a.divList[a.cache.elem][b]=c,d=!0):d=a.divList[a.cache.elem][b]);return d},add:function(b,c,d){if(b){var f="auto",h=b.className.split(" "),
k=function(b,c){return a._style(b,"zIndex")||a._style(b,"zIndex",c)};if(a.get.prefix.value(h,"",1)||d){var p=null,n=!1,m=!1,l=!1,r=!1,s=!1,q=!1,t=a.get.prefix.value(h,"",1),u=b.parentNode;if(t)switch(t){case "moveDiv":a._style(b,"cursor","move");a.set_eventListener(b,"mousedown",function(b){a.init.action(b,"move")});f=k(b,c);break;case "moveParentDiv":a._style(b,"cursor","move");a.set_eventListener(b,"mousedown",function(b){a.init.action(b,"moveparent")});b=b.parentNode;f=k(b,c);break;case "resizeDiv_tl":a._style(b,
...
"cursor","pointer");a.set_eventListener(b,"mousedown",function(b){a.on.minmax(b)});break;case "dropArea":a.dropArea.push([b,"global"])}for(a.get.prefix.value(h,"dropArea-",1)&&a.dropArea.push([b,a.get.prefix.value(h,"dropArea-",1)]);u;){if(u.className&&a.get.prefix.value(u.className.split(" "),"setLimit")){b!==u&&(p=u);break}u=u.parentNode}p||!a.get.prefix.value(h,"bodyLimit")&&!a.get.prefix.value(b.parentNode.className.split(" "),"bodyLimit")||(p=document.body);a.get.prefix.value(h,"dropLimit")?
n="global":a.get.prefix.value(h,"dropLimit-",1)&&(n=a.get.prefix.value(h,"dropLimit-",1));a.get.prefix.value(h,"dropMode-",1)&&(m=a.get.prefix.value(h,"dropMode-",1));a.get.prefix.value(h,"hideMove")&&a.get.prefix.value(h,"hideResize")?l="move_resize":a.get.prefix.value(h,"hideMove")?l="move":a.get.prefix.value(h,"hideResize")&&(l="resize");a.get.prefix.value(h,"keepAspect")&&b.clientWidth&&b.clientHeight&&(s=Math.abs(b.clientWidth/b.clientHeight));a.get.prefix.value(h,"showResize-",1)&&(r=a.get.prefix.value(h,
"showResize-",1),a.on.resize(b,!1),"dbclick"===r&&a.set_eventListener(b,"dblclick",function(b){a.on.resize(b,!0)}));switch(a.get.prefix.value(h,"saveSettings-",1)){case "position":q="position";break;case "size":q="size";break;case "position_size":q="position_size"}q&&!a.cache.unloadHandler&&(a.cache.unloadHandler=a.set_eventListener(g,"unload",a.on.unload));a.get.prefix.value(h,"loadSettings")&&(c=a.settings.load(b.id))&&("NaN"!==Number(c.left)&&"NaN"!==Number(c.top)&&(a.set.left(b,c.left),a.set.top(b,
c.top)),0<c.width&&0<c.height&&(a.set.width(b,c.width),a.set.height(b,c.height)));if(a.get.prefix.value(h,"moveParentDiv")||a.get.prefix.value(h,"moveDiv")||d)a.get.db.id(b).found||a.divList.push([b,p,!1,f,a.get.offset.relative(b).left,a.get.offset.relative(b).top,n,m,!1,l,r,s,q,b.clientWidth,b.clientHeight])}}},_style:function(a,c,d){if(a&&c&&a.style&&"undefined"!==typeof a.style[c])if(d)try{return a.style[c]=d}catch(f){return!1}else return""===a.style[c]?a.currentStyle?a.currentStyle[c]:g.getComputedStyle?
